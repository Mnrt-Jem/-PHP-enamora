    ...
    public static function buildBlog($helper, $blog, $image_type, $config)
    {
        ...
        if ($check_thumb && isset($blog['thumb']) && $blog['thumb'] != '') {
            if (isset($blog['thumb']) && $blog['thumb'] != '') {
                $blog['thumb_url'] = $url._ANBLOG_BLOG_IMG_URI_.'/b/'.$blog['thumb'];
                if (!file_exists(_ANBLOG_CACHE_IMG_DIR_.'b/'.$blog['id_anblog_blog'].'/'.$image_w.'_'.$image_h.'/'.$blog['thumb'])) {
                    @mkdir(_ANBLOG_CACHE_IMG_DIR_.'b/', 0777);
                    @mkdir(_ANBLOG_CACHE_IMG_DIR_.'b/'.$blog['id_anblog_blog'], 0777);
                    @mkdir(_ANBLOG_CACHE_IMG_DIR_.'b/'.$blog['id_anblog_blog'].'/'.$image_w.'_'.$image_h, 0777);
                    if (
                        ImageManager::resize(
                            _ANBLOG_BLOG_IMG_DIR_.'/b/'.$blog['thumb'],
                            _ANBLOG_CACHE_IMG_DIR_.'b/'.$blog['id_anblog_blog'].'/'.$image_w.'_'.$image_h.'/'.$blog['thumb'],
                            $image_w,
                            $image_h
                        )
                    ) {
                        // validate module
                        $blog['preview_thumb_url'] = _ANBLOG_CACHE_IMG_DIR_.'b/'.$blog['id_anblog_blog'].'/'.$image_w.'_'.$image_h.'/'.$blog['thumb'];
                    }
                }
                $blog['thumb_url'] = $url._ANBLOG_BLOG_IMG_URI_.'/b/'.$blog['thumb'];
                $blog['preview_thumb_url'] = $url._ANBLOG_CACHE_IMG_URI_.'b/'.$blog['id_anblog_blog'].'/'.$image_w.'_'.$image_h.'/'.$blog['thumb'];
            }
        } else {
            if ($blog['image']) {
                $blog['image_url'] = $url._ANBLOG_BLOG_IMG_URI_.'/b/'.$blog['image'];
                if (!file_exists(_ANBLOG_CACHE_IMG_DIR_.'b/'.$blog['id_anblog_blog'].'/'.$image_w.'_'.$image_h.'/'.$blog['image'])) {
                    @mkdir(_ANBLOG_CACHE_IMG_DIR_.'b/', 0777);
                    @mkdir(_ANBLOG_CACHE_IMG_DIR_.'b/'.$blog['id_anblog_blog'], 0777);
                    @mkdir(_ANBLOG_CACHE_IMG_DIR_.'b/'.$blog['id_anblog_blog'].'/'.$image_w.'_'.$image_h, 0777);
                    if (
                        ImageManager::resize(
                            _ANBLOG_BLOG_IMG_DIR_.'b/'.$blog['image'],
                            _ANBLOG_CACHE_IMG_DIR_.'b/'.$blog['id_anblog_blog'].'/'.$image_w.'_'.$image_h.'/'.$blog['image'],
                            $image_w,
                            $image_h
                        )
                    ) {
                        // validate module
                        $blog['preview_url'] = _ANBLOG_CACHE_IMG_DIR_.'b/'.$blog['id_anblog_blog'].'/'.$image_w.'_'.$image_h.'/'.$blog['image'];
                    }
                }
                $blog['image_url'] = $url._ANBLOG_BLOG_IMG_URI_.'/b/'.$blog['image'];
                $blog['preview_url'] = $url._ANBLOG_CACHE_IMG_URI_.'b/'.$blog['id_anblog_blog'].'/'.$image_w.'_'.$image_h.'/'.$blog['image'];
            }
        }*/
