...
/*             // REMOVE ID FROM URL
            ...
            $category = Anblogcat::findByRewrite(array('link_rewrite' => Tools::getValue('rewrite')));
        }
        ...
        if ($category->id_anblogcat && $category->active) {
            ...
            if ($category->image) {
                // validate module
                $category->image = $url._ANBLOG_BLOG_IMG_URI_.'/c/'.$category->image;
            }

            $leading_blogs = AnblogBlog::getListBlogs($category->id_anblogcat, $this->context->language->id, $p, $limit, 'date_add', 'DESC', array(), true);
